var assignedTaskListController = ["$scope", "$http", "$location", "$localStorage", function ($scope, $http, $location, $localStorage) {
    $scope.taskList = [];
    ...
    if ($localStorage.tasks != undefined) {
        ...
        $http.get("../AngularJSToDoList/getAllAssignedTaskList")
    ...
    $scope.syncAssignedTaskList = function () {
        $http.post("../AngularJSToDoList/syncAssignedTaskList", { tasks: angular.toJson($localStorage.tasks) })
        .then(function (response) {
