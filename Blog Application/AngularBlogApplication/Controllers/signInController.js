var signInController = ["$rootScope", "$scope", "$http", "$location", "$sessionStorage", "$filter", function ($rootScope, $scope, $http, $location, $sessionStorage, $filter) {
    ...
    $scope.saveSignInInformation = function () {
        $http.post("../AngularJSBlogApplication/isAlreadySignIn", { eMailID: $scope.signIn.EMailID })
        .success(function (response) {
            if (response == "Success") {
                $scope.signIn.Hobbies = $scope.hobbiesSelection;
                $scope.signIn.Orders = $scope.Orders;
                $http.post("../AngularJSBlogApplication/saveSignInInformation", { signIn: JSON.stringify($scope.signIn) })
                .success(function (response) {
                    $location.path("/Login");
                });
            }
