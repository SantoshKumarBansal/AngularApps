var postDetailsController = ["$rootScope", "$scope", "$http", "$location", "$sessionStorage", "$routeParams", function ($rootScope, $scope, $http, $location, $sessionStorage, $routeParams) {
    $scope.post = {};
    $scope.comments = {};
    ...
    $http.post("../AngularJSBlogApplication/getPost", { id: $routeParams.id })
    ...
    if (!$scope.isAlreadyRead) {
        ...
        $http.post("../AngularJSBlogApplication/readPost", { id: $routeParams.id })
    }

    $scope.likePost = function () {
        $scope.isAlreadyLiked = true;
        $sessionStorage.postLiked.push($routeParams.id);
        $http.post("../AngularJSBlogApplication/likePost", { id: $routeParams.id })
    ...
    $scope.unlikePost = function () {
        ...
        $http.post("../AngularJSBlogApplication/unlikePost", { id: $routeParams.id })
        .then(function (response) {
            $http.post("../AngularJSBlogApplication/getPost", { id: $routeParams.id })
                        .success(function (response) {
                            $scope.post = response[0];
    ...
    $scope.saveComment = function () {
        ...
        $http.post("../AngularJSBlogApplication/saveComment", { id: $routeParams.id, comments: angular.toJson(temp) })
        .then(function (response) {
            $scope.isShowCommentBlock = false;
            $http.post("../AngularJSBlogApplication/getPost", { id: $routeParams.id })
                        .success(function (response) {
